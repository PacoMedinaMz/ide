...
func declaraciones(nodo *d.Nodo){
	if(nodo != nil){
		...
		if(!checarTipo(nodo.TokenType.TypeLexema)){
			...
			lm.AgregarSemantico("Agregado a tabla de simbolos: " + nodo.TokenType.Lexema)
		}
		if(nodo.Bro != nil){
	...
	case tt.Asig:
			...
			case tt.Int:
				...
				if( err != nil){
					...
					lm.AgregarSemantico("Tabla modificada: " + nodo.Izq.TokenType.Lexema + " -> " + nodo.Der.AtrValor)
			...
			case tt.Bool:
				...
				if err != nil {
					...
					lm.AgregarSemantico("Tabla modificada: " + nodo.Izq.TokenType.Lexema + " -> " + nodo.Der.TokenType.Lexema)
...
func obtenerValorNumerico(nodo *d.Nodo)string{
	...
	var val int
	switch nodo.TokenType.TypeLexema {
	case tt.Plus: val = val1 + val2
	case tt.Minus: val = val1 - val2
	case tt.Div: val = val1 / val2
	case tt.Multi: val = val1 * val2	
	}
