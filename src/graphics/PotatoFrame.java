...
public class PotatoFrame {
    ...
    public void mostrarFrame() {
        pantalla.setIconImage(Toolkit.getDefaultToolkit().getImage(getClass().getResource("../assets/icon.png")));
    ...
    private static JPanel crearPanelP() {
        ...
        codeArea.setFont(new Font(letra_Fuente, Font.PLAIN, texto_size));
        ...
        scrollCodeArea.setSize(wid - f.porcentaje(wid, 60), hei - f.porcentaje(hei, 40));
        ...
        toolBarAnalizador.setLocation(wid - f.porcentaje(wid, 60) + (4 * paddingWID), paddingHEI);
        toolBarAnalizador.setSize(wid - f.porcentaje(wid, 45), paddingHEI);

        analizadorArea = new JTextPane();
        analizadorArea.setText("\n\n Area del Analizador Lexico.");
        analizadorArea.setForeground(texto_color);
        analizadorArea.setFont(new Font(letra_Fuente, Font.PLAIN, analizador_size));
        ...
        scrollAnalizadorArea.setLocation(wid - f.porcentaje(wid, 60) + (4 * paddingWID), (2 * paddingHEI));
        ...
        scrollErrorsArea.setSize(wid - (3 * paddingWID), f.porcentaje(hei, 20));
...
//                b.add(agregarItemMenu("Personalizar IDE"));
    ...
    private static JMenuItem agregarItemMenu(String x) {
        ...
        switch (x) {
            case "Nuevo archivo":
                ...
                item.addActionListener((ActionEvent e) -> {
                    if (codeArea.getText().trim().equals("")) {
                        ...
                        JOptionPane.showMessageDialog(null, "Guarda primero tu código");
                    }
                });
                break;
            case "Abrir archivo":
            ...
            case "Salir":
                item.addActionListener((ActionEvent e) -> {
                    int resp = JOptionPane.showConfirmDialog(null, "¿Desea cerrar fmaps studio?");
                    if (resp == 0) {
                        System.exit(0);
                    }
                });
...
//                break;
    ...
    private static boolean prerevisionCodigoPanel() {
        ...
        String nameFile = fa.seleccionarCarpeta();
        fa.guardarTxt(codeArea.getText(), nameFile); // Hay un error aqui, corrigelo omar del futuro
        /*
        O corrigelo tu Aldair, pero no creo que leas esto jejeje
        bueno el error tiene que ver con la ruta en la que se quiere guardar el archivo,
        no se porque no putas funciona.
         */
